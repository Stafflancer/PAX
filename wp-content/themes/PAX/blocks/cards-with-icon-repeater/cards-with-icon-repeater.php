...
$simple_card = get_field('card');
$buttons = get_field('buttons_group');

// Create id attribute allowing for custom "anchor" value.
$id = 'cards-with-icon-repeater' . $block['id'];
...
$section_class_name = 'cards-with-icon-repeater';
...
pax_display_block_background_options( $block, $container_args );
	...
	if(!empty($cards) || !empty($simple_card) && $add_card == 1){ 
		pax_card_with_icon($cards, $simple_card, $add_card);
	}
	if(!empty($buttons)){
