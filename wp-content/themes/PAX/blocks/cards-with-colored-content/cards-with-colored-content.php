...
$id = 'cards-with-colored-content' . $block['id'];
...
$section_class_name = 'cards-with-colored-content';
...
pax_display_block_background_options( $block, $container_args );
	...
	if(!empty($cards || !empty($simple_card) && $add_card == 1)){ 
		pax_card_with_colored_content($cards, $simple_card, $add_card);
	}
	if(!empty($buttons)){
