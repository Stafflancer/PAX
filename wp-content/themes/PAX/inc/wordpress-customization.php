...
function pax_unregister_tags() {
	unregister_taxonomy_for_object_type( 'post_tag', 'post' );
}

add_action( 'init', 'pax_unregister_tags' );
 ...
 * @return void
 */
function pax_filter_query( $query, $error = true ) {
	if ( is_search() ) {
...
//add_action( 'parse_query', 'pax_filter_query' );
 ...
 * @return void
 */
function pax_remove_search_widget() {
	unregister_widget( 'WP_Widget_Search' );
}

add_action( 'widgets_init', 'pax_remove_search_widget' );
...
function pax_remove_unnecessary_wordpress_menus() {
	global $submenu;
...
add_action( 'admin_menu', 'pax_remove_unnecessary_wordpress_menus', 999 );

/*** 7 - Show Featured Images in admin columns
 * @link https://www.isitwp.com/add-featured-thumbnail-to-admin-post-columns/
 * –––––––––––––––––––––––––––––––––––––––––––––––––– ***/
function posts_columns( $defaults ) {
	$defaults['pax_post_thumbs'] = __( 'Featured Image' );

	return $defaults;
}

function posts_custom_columns( $column_name, $id ) {
	if ( 'pax_post_thumbs' === $column_name ) {
 ...
 * @return mixed|void
 */
function pax_get_site_icon_url( $url, $size, $blog_id ) {
...
add_filter( 'get_site_icon_url', 'pax_get_site_icon_url', 10, 3 );
...
add_action( 'login_enqueue_scripts', 'pax_login_logo' );

function pax_login_logo_url() {
	return home_url();
}

add_filter( 'login_headerurl', 'pax_login_logo_url' );